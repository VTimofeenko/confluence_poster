...
def test_post_single_page_no_parent(make_one_page_config):
    ...
    result = run_with_config(
        config_file=config_file,
        input="Y\n" "N\n" "Y\n",  # do create page  # do not look for parent
    )  # do create in root
    assert result.exit_code == 0
...
def setup_page(tmp_path, record_pages):
    """Pre-creates a page"""
    config_file, config = generate_local_config(
        tmp_path, pages=1, filename="setup_page_config.toml"
    )
    result = run_with_config(
        input="Y\n"  # create page
        "N\n"  # do not look for parent
...
def test_post_force_overwrite_other_author(force_flag, tmp_path, setup_page):
    ...
    overwrite_config = mk_tmp_file(
        tmp_path,
        config_to_clone=str(overwrite_config),
        key_to_update="author",
        value_to_update=f"Fake: {fake_username}",
    )
    result = run_with_config(
        config_file=overwrite_config,
        pre_args=["--force"] * force_flag + ["--page-title", page_title],
    )
    assert result.exit_code == 0
    ...
    if force_flag:
        ...
        assert fake_username in result.stdout, (
