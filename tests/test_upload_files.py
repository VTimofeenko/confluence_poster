...
def setup_page(tmp_path, record_pages):
    ...
    post_page_run_cmd = generate_run_cmd(runner=runner, app=app, default_args=['post-page'])
    create_page = partial(run_with_config, default_run_cmd=post_page_run_cmd)
    result = create_page(config_file=config_path, record_pages=record_pages,
                         input="Y\nN\nY\n")
...
def test_upload_files_single_page_title_supplied(setup_page, gen_attachments):
    ...
    assert confluence_instance.get_attachments_from_content(created_page_id)['results'][0]['title'] == filename
    assert len(confluence_instance.get_attachments_from_content(page_id)['results']) == 0, \
...
def test_upload_files_multiple_pages_no_agree(setup_two_pages):
    ...
    result: Result = upload_files(config_file=two_page_config_file,
                                  input="N\n",
