...
def test_post_multiple_pages(make_two_pages, report):
    ...
    for page in config.pages:
        ...
        if with_report:
            ...
            assert (
                result.stdout.index("Created pages:\n")
...
def test_one_page_refuse_other_posted(make_two_pages):
    ...
    assert not page_created(
        page_title=config.pages[1].page_title
...
def test_one_author_correct_other_not(make_two_pages, tmp_path, force):
    ...
    *_, new_page_file = generate_fake_page(tmp_path)
    other_author_config_file = mk_tmp_file(
        tmp_path,
        filename="other_author.toml",
    ...
    result = run_with_config(config_file=other_author_config_file, pre_args=["--force"])
    assert result.exit_code == 0
    ...
    if force:
        result = run_with_config(config_file=config_file, pre_args=["--force"])
        assert result.exit_code == 0
        ...
        for page in config.pages:
            page_id = confluence_instance.get_page_by_title(
...
def test_one_page_parent_of_other(make_two_pages):
    ...
    assert result.exit_code == 0
    child_page_id = confluence_instance.get_page_id(
        space=parent_page.page_space, title=child_page.page_title
    )
    parent_page_id = confluence_instance.get_page_id(
        space=parent_page.page_space, title=parent_page.page_title
    )
    assert confluence_instance.get_parent_content_id(child_page_id) == parent_page_id
    assert all(
        [
            _ in get_pages_ids_from_stdout(result.stdout)
            for _ in [child_page_id, parent_page_id]
        ]
    )
