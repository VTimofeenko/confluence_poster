...
@pytest.mark.parametrize(
    ...
    [True, False, None],
    ids=[
        "User explicitly agrees to save file",
...
def test_single_dialog_new_config(tmp_path, monkeypatch, save_agree):
    ...
    assert (
        config_dialog(Path(path), ["author", "auth.url", "pages.page1.page_title"])
        == path.exists()
    )
    if file_created:
        assert (
...
def test_dialog_converts_filename_to_path(tmp_path, monkeypatch):
    ...
    for tested_type in [path_as_path, path_as_string]:
        ...
        assert config_dialog(
            tested_type, ["author", "auth.url", "pages.page1.page_title"]
        )
...
@pytest.mark.parametrize(
    "user_agrees_to_overwrite_file,mode",
    [(True, "insert"), (False, None)],
    ids=[
        "User adds a key to existing config",
        "User decides not to overwrite the file",
    ],
)
def test_single_dialog_existing_file_base(
    mode, user_agrees_to_overwrite_file, prepare_config_file, monkeypatch
):
    ...
    if user_agrees_to_overwrite_file:
        ...
        assert (
            parse(config.read_text())[node_path] == new_value
...
@pytest.mark.parametrize(
    ...
    ["Y", "N", ""],
    ids=[
        "User agrees to update parameter",
        "User refuses to update parameter",
        "User accepts the default choice to update parameter",
    ],
)
def test_single_dialog_existing_file_one_update(
    user_input, prepare_config_file, monkeypatch
):
    ...
    * If update happens - key is really updated
    ...
    if user_input in ["Y", ""]:
        assert (
            parse(config.read_text())["update_node"] == new_value
...
def test_single_dialog_existing_file_multiple_updates(
    user_updates_values, prepare_config_file, monkeypatch
):
    ...
    for user_answer, attr in zip([answer_1, answer_2], update_attrs):
        ...
        assert user_answer == (
            get_attribute_by_path(attribute_path=attr, config=parse(config.read_text()))
            == new_value
        )
...
def test_incremental_config_dialog(prepare_config_file, monkeypatch, capsys):
    ...
    assert (
        "Current content" not in captured.out
...
def test_sensitive_parameter_file_mode(tmp_path, monkeypatch, capsys):
    ...
    assert config_dialog(
        config_path, attributes=[DialogParameter("hidden param", hide_input=True)]
    )
    captured = capsys.readouterr()
